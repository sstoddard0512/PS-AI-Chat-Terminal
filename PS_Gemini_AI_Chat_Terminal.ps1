...
Provides interactive commands: /history, /clear, /retry, /config, /save, /media, /generate, /generate_from, /model, /imagemodel, /help.
...
function Upload-GeminiFile {
    ...
    $userAgent = "PowerShell-GeminiApi-Client/FileUploader-4.0.0" # Updated version
...
function Invoke-GeminiApi {
    ...
    Process {
        ...
        # --- Internal Helper: Add Media Part ---
        ...
        $currentRetry = 0; $response = $null; $userAgent = "PowerShell-GeminiApi-Client/Unified-4.0.0" # Updated version
...
function Parse-GeminiResponse {
    ...
    if (-not [string]::IsNullOrWhiteSpace($GeminiText)) {
        ...
        foreach ($line in $lines) {
            ...
            # Allow optional markdown (**) and optional numbering (d. ) at the start. Case-insensitive matching for labels.
...
function Handle-ChatCommand {
    ...
    switch -Regex ($TrimmedInput) {
        ...
        '^/config$' {
            ...
            $sessionConfig.GetEnumerator() | Sort-Object Name | ForEach-Object {
                $displayValue = if ($_.Value -ne $null) { ($_.Value | Out-String -Stream).Trim() } else { "(null)" }
                Write-Host ("{0,-25}: {1}" -f $_.Name, $displayValue)
            }
            Write-Host "---------------------------" -ForegroundColor Yellow
            $commandResult.CommandExecuted = $true; $commandResult.SkipApiCall = $true
        }
        ...
        '^/model(\s+(\S+))?$' {
             ...
             if ($Matches[2]) { $newModel = $Matches[2].Trim(); $sessionConfig.Model = $newModel; Write-Host "Model changed to '$newModel'." -F Yellow }
                 ...
                 $modelInput = Read-Host "Enter number or custom name"; if ($modelInput -match '^\d+$' -and [int]$modelInput -ge 1 -and [int]$modelInput -le $geminiModelOptions.Count) { $sessionConfig.Model = $geminiModelOptions[[int]$modelInput - 1] } elseif ($modelInput) { $sessionConfig.Model = $modelInput.Trim() } else { Write-Warning "No change."; $modelInput = $null }
                 if ($modelInput) { Write-Host "Model changed to '$($sessionConfig.Model)'." -F Yellow }
             }
        ...
        '^/imagemodel(\s+(\S+))?$' {
             ...
             if ($Matches[2]) { $newModel = $Matches[2].Trim(); $sessionConfig.VertexImageModel = $newModel; Write-Host "Vertex Image model changed to '$newModel'." -F Yellow }
                 ...
                 $modelInput = Read-Host "Enter number or custom name"; if ($modelInput -match '^\d+$' -and [int]$modelInput -ge 1 -and [int]$modelInput -le $imagenModelOptions.Count) { $sessionConfig.VertexImageModel = $imagenModelOptions[[int]$modelInput - 1] } elseif ($modelInput) { $sessionConfig.VertexImageModel = $modelInput.Trim() } else { Write-Warning "No change."; $modelInput = $null }
                 if ($modelInput) { Write-Host "Vertex Image model changed to '$($sessionConfig.VertexImageModel)'." -F Yellow }
             }
             $SessionConfigRef.Value = $sessionConfig # Write changes back
             $commandResult.CommandExecuted = $true; $commandResult.SkipApiCall = $true
        }
        '/(generate|image)(\s+(.+))?$' { # Prompt is optional
            ...
            $imageGenPrompt = $null; $cmdName = $Matches[1]
            if ($Matches[3]) { $imageGenPrompt = $Matches[3].Trim() }
            else { $imageGenPrompt = Read-Host "Enter prompt for /$cmdName"; if ([string]::IsNullOrWhiteSpace($imageGenPrompt)) { Write-Warning "Prompt cannot be empty. Command cancelled."; $commandResult.CommandExecuted = $true; $commandResult.SkipApiCall = $true; return $commandResult } }
        ...
        '^/generate_from(\s+(.+))?$' { # Path is optional
            ...
            $inputPath = $null
            if ($Matches[2]) { $inputPath = $Matches[2].Trim().Trim('"').Trim("'") }
            else { $inputPath = Read-Host "Enter source image file/folder path for /generate_from"; if ([string]::IsNullOrWhiteSpace($inputPath)) { Write-Warning "Path cannot be empty. Command cancelled."; $commandResult.CommandExecuted = $true; $commandResult.SkipApiCall = $true; return $commandResult } $inputPath = $inputPath.Trim().Trim('"').Trim("'") }
    ...
    try {
        while ($true) {
            ...
            if ($isFirstTurn) {
                ...
                if ($sessionConfig.MediaFolder -and $StartPrompt) { # Check StartPrompt again
